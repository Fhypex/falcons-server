...
public interface JpaReservationRepository extends JpaRepository<ReservationEntity, UUID> {
    ...
    @Query("SELECT COUNT(a) > 0 FROM ReservationEntity a WHERE a.courtId = :courtId AND a.date = :date AND a.hour = :hour AND a.status != 'CANCELLED' AND a.status != 'REJECTED'")
    ...
    List<ReservationEntity> findByOwnerIdAndStatus(UUID ownerId, ReservationStatus status);
}
